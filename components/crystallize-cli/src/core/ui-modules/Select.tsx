...
export function Select<T>({ options, onSelect, styles, defaultSelectedIndex = 0 }: Props<T>) {
    ...
    const lastDisplayedIndex = options.findIndex((option: Option<T>) => option === optionsToDisplay[optionsToDisplay.length - 1]);
    ...
    return <Box flexDirection="column">
        {optionsToDisplay.map((option: Option<T>, index: number) => {
            return <Box flexDirection="row" marginY={styles?.compact ? 0 : 1} key={index}>
