_crystallize_completions() {
  ...
  case "${cmd}" in
    ...
    mass-operation)
        if [[ "${COMP_CWORD}" -eq 2 ]]; then
                local options="run dump-content-model execute-mutations add-operation validate ${default_options}"
                COMPREPLY=($(compgen -W "${options}" -- "${cur}"))
                return 0
        fi
        case "${subcmd}" in
    ...
    plugin)
        if [[ "${COMP_CWORD}" -eq 2 ]]; then
                local options="create keygen encrypt-secret decrypt-payload ${default_options}"
                COMPREPLY=($(compgen -W "${options}" -- "${cur}"))
                return 0
        fi
        case "${subcmd}" in
